...
export default async function handler(req,res){
 try{
  if(req.method==='GET') return res.status(200).json({ok:true,data:await sb('files?select=*')});
  if(req.method==='POST') return res.status(200).json({ok:true,data:await sb('files',{method:'POST',body:JSON.stringify(req.body)})});
