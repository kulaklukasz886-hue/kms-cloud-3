...
async function sb(path, options = {}) {
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    throw new Error('Brak SUPABASE_URL lub SUPABASE_ANON_KEY w Vercel Environment Variables.');
  }
  ...
  if (!res.ok) {
    throw new Error(data?.message || data?.error || text || `Supabase HTTP ${res.status}`);
  }
...
function mapMaterial(orderId, m) {
  return {
    order_id: orderId,
    type: String(m.type || ''),
    material: String(m.material || ''),
    quantity: Number(m.qty ?? m.quantity ?? 0) || 0,
    unit: String(m.unit || ''),
    supplier: String(m.producer || m.supplier || m.glassSupplier || ''),
    status: String(m.status || 'Do zamówienia'),
    related_type: String(m.relatedType || m.related_type || '')
  };
}

function mapTask(orderId, t) {
  return {
    order_id: orderId,
    station: String(t.station || t.name || t.task_name || 'Produkcja'),
    task_name: String(t.name || t.task_name || t.station || 'Zadanie'),
    status: String(t.status || 'Do wykonania'),
...
function mapFile(orderId, f) {
  return {
    order_id: orderId,
    file_name: String(f.name || f.file_name || 'plik'),
    file_url: String(f.url || f.file_url || ''),
    station: String(f.station || 'Zamówienie'),
...
async function loadChildren(orders) {
  ...
  return orders.map(o => ({
    ...o,
    materials: safeArray(materials).filter(x => x.order_id === o.id),
    tasks: safeArray(tasks).filter(x => x.order_id === o.id),
    files: safeArray(files).filter(x => x.order_id === o.id)
  }));
}

async function rollbackOrder(orderId) {
  if (!orderId) return;
  try { await sb(`orders?id=eq.${orderId}`, { method: 'DELETE' }); } catch (e) { console.error('rollback failed', e); }
}
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        throw new Error('Zlecenie cofnięte. Błąd zapisu materiałów/zadań/plików: ' + childError.message);
      }
